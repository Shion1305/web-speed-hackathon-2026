#!/usr/bin/env node
/**
 * Generates a fully static dist/term.html for the /term page.
 * - No React, no JS bundle
 * - Font preload + @font-face injected directly as DOM nodes in <head>
 * - Minimal inline styles for layout (no CSS bundle dependency)
 */
    ...
    <style>
      @font-face {
        ...
        src: url(/fonts/ReiNoAreMincho-Regular-Subset.woff2) format("woff2"),
             url(/fonts/ReiNoAreMincho-Regular.otf) format("opentype");
        font-weight: normal;
      }
      @font-face {
        font-family: "Rei no Are Mincho";
        ...
        src: url(/fonts/ReiNoAreMincho-Heavy-Subset.woff2) format("woff2"),
             url(/fonts/ReiNoAreMincho-Heavy.otf) format("opentype");
        font-weight: bold;
      }
      *, *::before, *::after { box-sizing: border-box; }
      ...
      @media (min-width: 768px) {
        article { padding: 0.5rem 1rem 4rem; }
      }
      h1 {
        font-family: "Rei no Are Mincho", serif;
        font-size: 1.875rem;
        line-height: normal;
        font-weight: bold;
  ...
  <body>
    <article>
      ...
      <ol>
        ...
        <li>ユーザーは、いかなる場合にも、ユーザーIDおよびパスワードを第三数に譲渡または貸与し、もしくは第三者と共用することはできません。当社は、ユーザーIDとパスワードの組み合わせが登録情報と一致してサインインされた場合には、そのユーザーIDを登録しているユーザー自身による利用とみなします。</li>
...
fs.writeFileSync(path.join(DIST_PATH, "term.html"), html, "utf8");
console.log("Generated dist/term.html");
